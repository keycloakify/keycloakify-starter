...
import { Button, Center, Text, Select } from "@chakra-ui/react";
...
export default function OrgSelectPage(
  ...
  return (
    ...
    >
      ...
      <Center>
        <Text fontSize="md">
          Select one of the organizations available to you from the list below
          to be used in this integration
        </Text>
      </Center>
      <form action={kcContext.url.loginAction} method="post">
        <Center mt={2}>
          <Select name="organization_key">
            {organizations &&
              organizations.map((org, i) => (
                <option value={org.segment_identifier} selected={i === 0}>
                  {" "}
                  {org.name}
                </option>
              ))}
